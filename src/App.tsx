...
const searchShopeeProducts = async (keyword: string): Promise<Product[]> => {
  try {
    ...
    const items: ShopeeNode[] = data?.data?.productOfferV2?.nodes || [];
  ...
  return (
    ...
    >
      ...
      <div style={{ position: 'relative' }}>
        <img
          ...
          onError={(e) => {
            ...
            target.src = 'https://placehold.co/280x160/fce4f3/e91e8c?text=Produto';
          }}
        />
        {product.discount > 0 && (
          <div style={{
      ...
      <div style={{ padding: 14 }}>
        <div style={{ fontSize: 11, color: TEXT_LIGHT, marginBottom: 4, fontWeight: 600 }}>
          {product.shop}
        </div>
        <p style={{
          fontSize: 13, color: TEXT, margin: '0 0 8px', lineHeight: 1.4, fontWeight: 500,
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 3, marginBottom: 8 }}>
          ...
          <span style={{ fontSize: 11, color: '#b45309', marginLeft: 3, fontWeight: 600 }}>
...
export default function BoxePromocoes() {
  ...
  const historyRef = useRef<GeminiPart[]>([]);
  ...
  const sendMessage = async (text?: string) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...
        {
          role: 'assistant',
          text: aiText || 'Aqui estão as melhores opções que encontrei para você! 🛍️',
      ...
      setMessages((prev) => [
        ...prev,
        { role: 'assistant', text: 'Ops! Algo deu errado. Tente novamente! 😊', products: null },
      ]);
    }
    setLoading(false);
  };
  ...
  return (
    <div style={{
      ...
      fontFamily: 'system-ui, sans-serif', color: TEXT,
      display: 'flex', flexDirection: 'column',
    }}>
      {/* Faixa decorativa topo */}
      <div style={{ height: 4, background: `linear-gradient(90deg, ${PINK}, ${PINK_DARK}, ${PINK})` }} />

      {/* Header */}
      <header style={{
        position: 'sticky', top: 0, zIndex: 50,
        background: '#fff', borderBottom: `1px solid ${PINK_BORDER}`,
        ...
        <div style={{
          ...
          fontSize: 22, boxShadow: '0 4px 12px #e91e8c33',
        }}>🛍️</div>
        <div>
          <div style={{ fontSize: 18, fontWeight: 900, color: PINK, letterSpacing: -0.5 }}>
            Box e Promoções
          </div>
          <div style={{ fontSize: 11, color: TEXT_LIGHT, display: 'flex', alignItems: 'center', gap: 5 }}>
            <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#22c55e', boxShadow: '0 0 6px #22c55e' }} />
            Assistente IA online
          </div>
        </div>
        <div style={{ marginLeft: 'auto', fontSize: 10, color: '#ccc', textAlign: 'right' }}>
      ...
      <div style={{
        ...
        {messages.length === 1 && (
          <div style={{ marginBottom: 28 }}>
            <p style={{ fontSize: 12, color: TEXT_LIGHT, letterSpacing: 2, marginBottom: 10, fontWeight: 600 }}>
              TOQUE PARA COMEÇAR
            </p>
              ...
              {SUGGESTIONS.map((s, i) => (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 100, padding: '8px 16px', color: PINK_DARK,
          ...
          <div key={i} style={{ marginBottom: 24 }}>
            <div style={{
              ...
              <div style={{
                ...
                background: msg.role === 'user'
                  ...
                  : CARD_BG,
                border: msg.role === 'user' ? 'none' : `1px solid ${PINK_BORDER}`,
                borderRadius: msg.role === 'user' ? '18px 18px 4px 18px' : '18px 18px 18px 4px',
                padding: '12px 16px',
                color: msg.role === 'user' ? '#fff' : TEXT,
                fontSize: 14, lineHeight: 1.6,
                boxShadow: msg.role === 'user' ? '0 4px 12px #e91e8c33' : '0 1px 4px #e91e8c11',
              }}>
                {msg.text}
              </div>
              {msg.role === 'user' && (
            ...
            {msg.products && msg.products.length > 0 && (
              <div style={{ marginTop: 16, paddingLeft: 44 }}>
                <p style={{ fontSize: 11, color: PINK, letterSpacing: 2, marginBottom: 12, fontWeight: 700 }}>
        ...
        {loading && (
          <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start', marginBottom: 24 }}>
            ...
            <div style={{
              background: CARD_BG, border: `1px solid ${PINK_BORDER}`,
      ...
      <div style={{
        ...
        background: '#fff', borderTop: `1px solid ${PINK_BORDER}`,
        padding: '14px 16px', boxShadow: '0 -2px 12px #e91e8c11',
      }}>
        <div style={{ maxWidth: 780, margin: '0 auto', display: 'flex', gap: 10 }}>
          <input
            ...
            style={{
              flex: 1, background: PINK_LIGHT, border: `1px solid ${PINK_BORDER}`,
              borderRadius: 12, padding: '13px 18px', color: TEXT,
              fontSize: 14, fontFamily: 'inherit', outline: 'none', transition: 'border-color 0.2s',
            }}
            onFocus={(e) => (e.currentTarget.style.borderColor = PINK)}
            onBlur={(e) => (e.currentTarget.style.borderColor = PINK_BORDER)}
          />
          <button
            onClick={() => sendMessage()}
            ...
            style={{
              ...
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              boxShadow: loading || !input.trim() ? 'none' : '0 4px 12px #e91e8c33',
            }}
          >
            <SendIcon />
          </button>
